...
public class BlockShapeSpawner : Singleton<BlockShapeSpawner> 
{
	...
	void SetupPreviousSessionShapes()
	{
		if (GameBoardGenerator.Instance.previousSessionData != null) {
			List<int> shapes = GameBoardGenerator.Instance.previousSessionData.shapeInfo.Split(',').Select(System.Int32.Parse).ToList();
	...
	private void AddLatter(GameObject shape)
	{
		...
		{
			TextMeshProUGUI ltr = Instantiate(letterObj, shape.transform.GetChild(i));
			ltr.text = l;
		}
	}
