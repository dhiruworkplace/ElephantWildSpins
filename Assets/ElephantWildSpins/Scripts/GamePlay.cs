...
public class GamePlay : Singleton<GamePlay>, IPointerDownHandler, IPointerUpHandler, IBeginDragHandler, IDragHandler
{
	...
	[SerializeField] private int MaxAllowedVideoWatchRescue = 3;
	...
	void Start()
	{
		...
		Invoke(nameof(CheckForHelp), 0.5F);
		#endregion
	}
	...
	public void OnGameOver()
	{
		...
		// Calculate 0.5% of the current score and convert it to an integer
		int bonusScore = Mathf.RoundToInt(ScoreManager.Instance.Score * 0.005f);

		// Pass the calculated score to SetLevelScore
		gameOverScreen.GetComponent<GameOver>().SetLevelScore(ScoreManager.Instance.Score, bonusScore);
